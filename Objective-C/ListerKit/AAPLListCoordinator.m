...
- (void)copyFileToDocumentsDirectory:(NSURL *)fromURL {
    ...
    [fileCoordinator coordinateWritingItemAtURL:fromURL options:NSFileCoordinatorWritingForMoving writingItemAtURL:toURL options:NSFileCoordinatorWritingForReplacing error:&error byAccessor:^(NSURL *sourceURL, NSURL *destinationURL) {
    ...
    if (!success) {
        ...
        NSLog(@"Couldn't move file: %@ to: %@. Error: %@.", fromURL.absoluteString, toURL.absoluteString, error.description);
    }
}
